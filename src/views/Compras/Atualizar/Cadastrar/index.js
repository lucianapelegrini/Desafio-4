...
export const NovaCompra = () => {
    ...
    const cadCompra = async e => {
        ...
        await axios.post(api+"/compras/novacompra", compra, { headers })
        .then((response) => {
            setStatus({
                type: "success",
    ...
    return (
        <Container>
            ...
            {status.type === "success" ? <Alert color="warning">{status.message}</Alert> : ""}
            ...
            <Form className="p-2" onSubmit={cadCompra}>
                ...
                <Button style={{ borderRadius: 40 }} outline value="AdicionarItem" onClick={botaoAddItem}><FiPlusCircle></FiPlusCircle> item</Button>
