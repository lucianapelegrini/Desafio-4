...
export const AtualizarPedido = (props) => {
    ...
    const atualizar = async e => {
        ...
        await axios.put(api + "/pedido/" + PedidoId + "/atualizarpedido", pedido, {headers})
        .then((response) => {
            setStatus({
                type: "success",
                message: response.data.message
        ...
        <div>
            <Container>
                ...
                <Form className="p-2" onSubmit={atualizar}>
                    ...
                    <Button type="button" className="m-2" outline color="warning" onClick={limparInput}>Limpar</Button>
                </Form>
            </Container>
        </div>
    );
