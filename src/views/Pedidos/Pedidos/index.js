...
export const Pedidos = () => {
    ...
    const excluir = async (id) => {
        ...
        await axios.get(api+"/pedido/"+id+"/excluirpedido", {headers})
        .then((response) => {
            setStatus({
                type: "success",
                message: response.data.message
                    ...
                    <tbody>
                        {data.map(item => (
                            <tr key={item.id}>
                                ...
                                <td>{item.data}</td>
                                <td>
                                    <Link to={"/pedido/" + item.id + "/servico"} className="btn btn-outline-info btn-sm">Consultar</Link>
