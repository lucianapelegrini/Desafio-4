...
export const NovoPedido = () => {
    ...
    const cadPedido = async e => {
        ...
        await axios.post(api+"/pedidos/novopedido", pedido, {headers})
        .then((response) => {
            setStatus({
                type: "success",
        ...
        await axios.post(api+"/pedidos/"+PedidoId + "/novo-item-pedido", item, {headers});
    };
    ...
    return (
        <Container>
            ...
            <Form className="p-2" onSubmit={cadPedido}>
                ...
                <Button style={{ borderRadius: 40 }} outline value="AdicionarItem" onClick={addItem}><FiPlusCircle></FiPlusCircle> item</Button>

                <div className="d-flex justify-content-between">
                    <div></div>
                    <Col className="d-flex flex-row-reverse">
                        <Button type="submit" outline color="success">Cadastrar</Button>
                        <Link to="/pedidos" className="btn btn-outline-warning btn">Voltar</Link>
                    </Col>
                </div>
            </Form>
        </Container>
