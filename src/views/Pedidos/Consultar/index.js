...
export const Pedido = (props) => {
    ...
    const excluirItem = async (idPed, idServ) => {
        ...
        await axios.get(api+"/itens_pedido/"+idPed+"/"+idServ+"/excluiritem", {headers})
        .then((response) => {
            setStatus({
                type: "success",
                message: response.data.message
                    ...
                    <tbody>
                        {data.map(item => (
                            <tr key={item.id}>
                                ...
                                <td>
                                    <Link to={"/servicos/" + item.ItemPedido.ServicoId + "/pedido"} className="btn btn-outline-info btn-sm">Consultar</Link>
